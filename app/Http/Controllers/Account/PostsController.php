...
class PostsController extends AccountBaseController
{
    ...
    public function getStudentsList()
    {
        ...
        $stdnts = User::select('name','country_code','phone')->where(['user_type_id' => 2, 'blocked' => 0])->get();

        $data['students'] = $stdnts;

        // Meta Tags
        MetaTag::set('title', t('Students List'));
        MetaTag::set('description', t('Students registered on :app_name', ['app_name' => config('settings.app.app_name')]));

        return view('account.students', $data);
    }
